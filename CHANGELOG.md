...
- **Card de "Productos" del Home mostraba un error para usuarios `ROLE_COBRADOR`**: esa card se muestra a cualquier usuario logueado (a diferencia de las de Ventas/Usuarios, que sí filtran por rol), pero `GET /api/productos/cantidad-productos` estaba restringido a `ROLE_ADMIN`/`ROLE_INVENTARIO` — un usuario solo-`ROLE_COBRADOR` (que sí puede ver el listado completo de productos) recibía un 403 al cargar el Home y el catchError del servicio disparaba un `swal` de error crudo. Se agregó `ROLE_COBRADOR` al endpoint; además el servicio ahora degrada a 0 ante cualquier error en vez de interrumpir con un `swal`, al ser un contador informativo sin nada crítico que reportar.
