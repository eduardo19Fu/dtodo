...
class VentasProductoTemplateTest {

    @Test
    void compilaYLlenaPlantillaSinDatos() throws Exception {
        try (InputStream template = getClass().getResourceAsStream("/reports/ventas_producto.jrxml")) {
    ...
    void generaPdfDeMuestraConFilasYTotales() throws Exception {
        try (InputStream template = getClass().getResourceAsStream("/reports/ventas_producto.jrxml")) {
            ...
            String rutaMuestra = System.getProperty("reporte.ventas.producto.muestra.path");
    ...
    private List<Map<String, ?>> filas() {
        ...
        rows.add(fila("75010001", "Cuaderno universitario", "Librería", "48", "1200.00", "60.00", "1140.00"));
        rows.add(fila("75010002", "Marcador permanente azul", "Oficina", "32", "640.00", "20.00", "620.00"));
        rows.add(fila("75010003", "Detergente multiusos", "Limpieza", "18", "540.00", "0.00", "540.00"));
        return rows;
    }
