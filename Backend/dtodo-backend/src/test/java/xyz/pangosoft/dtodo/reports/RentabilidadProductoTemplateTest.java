...
class RentabilidadProductoTemplateTest {

    @Test
    void compilaYLlenaPlantillaSinDatos() throws Exception {
        try (InputStream template = getClass().getResourceAsStream("/reports/rentabilidad_producto.jrxml")) {
            ...
            assertTrue(Arrays.stream(report.getFields())
                    .anyMatch(field -> "margen_porcentaje".equals(field.getName())));
    ...
    void generaPdfDeMuestraConFilasYTotales() throws Exception {
        try (InputStream template = getClass().getResourceAsStream("/reports/rentabilidad_producto.jrxml")) {
            ...
            guardarMuestra("reporte.rentabilidad.producto.pdf.muestra.path", pdf);
        }
    }
    ...
    void generaXlsxDeMuestraConFilasYTotales() throws Exception {
        try (InputStream template = getClass().getResourceAsStream("/reports/rentabilidad_producto.jrxml")) {
            ...
            assertXlsx(xlsx);
            guardarMuestra("reporte.rentabilidad.producto.xlsx.muestra.path", xlsx);
        }
    }
    ...
    private List<Map<String, ?>> filas() {
        ...
        rows.add(fila("75010001", "Cuaderno universitario", "Librería", "48",
                "1140.00", "720.00", "420.00", "0.368421"));
        rows.add(fila("75010002", "Marcador permanente azul", "Oficina", "32",
                "620.00", "352.00", "268.00", "0.432258"));
        rows.add(fila("75010003", "Detergente multiusos", "Limpieza", "18",
                "540.00", "378.00", "162.00", "0.300000"));
        return rows;
    }
    ...
    private Map<String, Object> fila(
        ...
        row.put("venta_neta", new BigDecimal(venta));
        row.put("costo_estimado", new BigDecimal(costo));
        row.put("margen", new BigDecimal(margen));
        row.put("margen_porcentaje", new BigDecimal(porcentaje));
        return row;
    }
    ...
    private byte[] exportarXlsx(JasperPrint print) throws Exception {
        try (ByteArrayOutputStream output = new ByteArrayOutputStream()) {
            ...
            configuration.setSheetNames(new String[] { "Rentabilidad por producto" });
